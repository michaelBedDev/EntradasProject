...
export default async function MisEventosPage() {
  ...
  const eventos = await getEventosByOrganizador();

  // Función para obtener el color del badge según el estado
  const getBadgeVariant = (status: string) => {
    switch (status) {
      case EventStatus.Aprobado:
        return "success";
              ...
              <TableBody>
                {eventos.map((evento) => (
                  <TableRow key={evento.id}>
                    ...
                    <TableCell>
                      <Badge variant={getBadgeVariant(evento.status) as any}>
                        {evento.status === EventStatus.Aprobado
                          ? "Aprobado"
                          : evento.status === EventStatus.Pendiente
                          ? "Pendiente"
                          : "Cancelado"}
                      </Badge>
                    </TableCell>
